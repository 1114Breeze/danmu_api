...
[![GitHub](https://img.shields.io/badge/-GitHub-181717?logo=github)](https://github.com/1114Breeze/danmu_api)
![GitHub forks](https://img.shields.io/github/forks/1114Breeze/danmu_api)
![GitHub Repo stars](https://img.shields.io/github/stars/1114Breeze/danmu_api)
![GitHub License](https://img.shields.io/github/license/1114Breeze/danmu_api)
...
[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/1114Breeze/danmu_api)
...
- **UI界面-后台配置管理系统**：支持通过UI执行一些操作（详细见 [UI 系统使用说明](https://github.com/1114Breeze/danmu_api/tree/main/danmu_api/ui/README.md) ），包括：
  - 配置预览
  - 日志查看
  - 接口调试/弹幕测试
  - 推送弹幕
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/1114Breeze/danmu_api&project-name=danmu_api&repository-name=danmu_api)

**注意**：请将按钮链接中的 `https://github.com/1114Breeze/danmu_api` 替换为你的实际 Git 仓库地址。编辑 `README.md` 并更新链接后，推送到仓库，点击按钮即可自动克隆和部署。
...
<a href="https://app.netlify.com/start/deploy?repository=https://github.com/1114Breeze/danmu_api"><img src="https://www.netlify.com/img/deploy/button.svg"></a>
...
### 一键部署
[![使用 EdgeOne Pages 部署](https://cdnstatic.tencentcs.com/edgeone/pages/deploy.svg)](https://console.cloud.tencent.com/edgeone/pages/new?template=https://github.com/1114Breeze/danmu_api&project-name=danmu-api&root-directory=.%2F&env=TOKEN)
...
> 
> 也可直接用国际站的部署按钮一键部署，默认选择"全球可用区（不含中国大陆）" [![使用 EdgeOne Pages 部署](https://cdnstatic.tencentcs.com/edgeone/pages/deploy.svg)](https://edgeone.ai/pages/new?template=https://github.com/1114Breeze/danmu_api&project-name=danmu-api&root-directory=.%2F&env=TOKEN)
> 
...
[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/1114Breeze/danmu_api)
...
配合 dd-danmaku 扩展新增对 Emby Web 端弹幕的支持，具体使用方法参考 [PR #98](https://github.com/1114Breeze/danmu_api/pull/98) 。
...
> 
> 小幻在使用时可能出现掉匹配无法加载弹幕的问题，详见[这个issue](https://github.com/1114Breeze/danmu_api/issues/33)，可以通过配置环境变量`UPSTASH_REDIS_REST_URL`和`UPSTASH_REDIS_REST_TOKEN`，开启upstash redis存储解决
> 
> 有很多人问FW能不能用，FW推荐直接使用插件，如果非要使用，则可以配合 `https://raw.githubusercontent.com/1114Breeze/ForwardWidgets/refs/heads/main/widgets.fwd` 里的`danmu_api`插件使用
...
| DEPLOY_PLATFROM_ACCOUNT    | 【可选】部署账号ID，调用部署服务API需要，配置后可使用UI界面配置服务，不同部署平台获取方式可查看 [部署平台环境变量配置指南](https://github.com/1114Breeze/danmu_api/tree/main/danmu_api/ui/README.md#部署平台环境变量配置指南) ，docker部署和本地node部署并不需要配置      |
| DEPLOY_PLATFROM_PROJECT    | 【可选】部署项目名称，调用部署服务API需要，配置后可使用UI界面配置服务，不同部署平台获取方式可查看 [部署平台环境变量配置指南](https://github.com/1114Breeze/danmu_api/tree/main/danmu_api/ui/README.md#部署平台环境变量配置指南) ，docker部署和本地node部署并不需要配置       |
| DEPLOY_PLATFROM_TOKEN    | 【可选】部署平台token，调用部署服务API需要，配置后可使用UI界面配置服务，不同部署平台获取方式可查看 [部署平台环境变量配置指南](https://github.com/1114Breeze/danmu_api/tree/main/danmu_api/ui/README.md#部署平台环境变量配置指南) ，docker部署和本地node部署并不需要配置       |
...
- 有任何问题，如部署/环境变量配置等，可通过deepwiki对本项目进行提问，链接入口：https://deepwiki.com/1114Breeze/danmu_api ，其中项目内容一般每周刷新一次
...
### 贡献者
<a href="https://github.com/1114Breeze/danmu_api/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=1114Breeze/danmu_api" alt="contributors" />
</a>

### 📈项目 Star 数增长趋势
#### Star History
[![Star History Chart](https://api.star-history.com/svg?repos=1114Breeze/danmu_api&type=Date)](https://www.star-history.com/#1114Breeze/danmu_api&Date)
